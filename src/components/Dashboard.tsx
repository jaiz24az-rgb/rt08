...
import { Balance, LedgerEntry } from '../types';
import { 
  Coins, 
  TrendingUp, 
  TrendingDown, 
...
export default function Dashboard({ kas, updateKas, ledger, addLedgerEntry, isLoggedIn }: DashboardProps) {
  const totalRT = kas.rtTunai + kas.rtPettyCash + kas.rtBank;
  const totalRombong = kas.rombongTunai + kas.rombongBank;
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex gap-2">
            {isLoggedIn ? (
              <button 
                ...
                className="flex items-center gap-2 bg-gradient-to-r from-sky-500 to-blue-600 hover:from-sky-600 hover:to-blue-700 text-white font-semibold px-4 py-2.5 rounded-xl transition duration-200 text-sm shadow-md shadow-sky-500/20 active:scale-95 cursor-pointer"
                id="btn-quick-tx"
              >
              ...
              <p className="text-xs bg-slate-800 border border-slate-700 text-amber-300 px-3 py-2.5 rounded-xl flex items-center gap-1.5 leading-relaxed max-w-xs">
                <AlertCircle className="w-3.5 h-3.5 shrink-0 text-amber-400" />
                Masuk/Login sebagai Admin di menu bawah untuk menambah data & ubah saldo.
              </p>
            )}
          </div>
        </div>
        ...
        <div className="mt-8 pt-6 border-t border-slate-800">
          ...
          <div className="flex flex-wrap justify-start gap-x-6 gap-y-2 mt-4 text-xs font-medium">
            <div className="flex items-center gap-2">
              ...
              <span className="text-slate-300">Total Sektor RT:</span>
              <span className="text-sky-300 font-mono">Rp {totalRT.toLocaleString('id-ID')}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-emerald-400 shrink-0" />
              <span className="text-slate-300">Total Sektor Rombong:</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {/* Pengeluaran Stats */}
        <div className="p-5 bg-white border border-slate-200 rounded-2xl flex items-center gap-4 shadow-xs">
          <div className="p-3.5 rounded-xl bg-rose-50 text-rose-600 border border-rose-100 font-bold">
